...
describe("Navbar", () => {
  it("renders with navigation links", () => {
    ...
    expect(getByText("Home").closest("a")).toHaveAttribute("href", `/home?username=${username}`);

    // Assert that the Catalogue link is rendered correctly
    expect(getByText("Catalogue").closest("a")).toHaveAttribute("href", `/catalogue?username=${username}`);

    // Assert that the Add Waste link is rendered correctly
    expect(getByText("Add Waste").closest("a")).toHaveAttribute("href", `/addwaste?username=${username}`);

    // Assert that the Favourite link is rendered correctly
    expect(getByText("Favourite").closest("a")).toHaveAttribute("href", "/favourite");

    // Assert that the Profile link is rendered correctly
    expect(getByText("Profile").closest("a")).toHaveAttribute("href", `/profile?username=${username}`);
